function [val, dval] = lml_mvm_map(cov, y, hyp, Z, lambda, precond, S)
...
if isfield(hyp, 'lik')
    sigma2 = exp(2*hyp.lik) + lambda; % Add lambda to sigma^2 and compute the new "sigma"
    ...
    if length(hyp.lik) == 2 % Use one sigma for the values, one for the derivatives
        ...
        sigma2 = [sigma2(1)*ones(n,1); sigma2(2)*ones(n*d, 1)];
    else
...
if precond
    if isempty(S) % No scaling
        if ~isempty(dd) && ~isempty(get_row)
            [L, pp] = pchol_handles(get_row, dd, 1e-6, ceil(min(nd/10, 400)));
            P = pchol_solve(pp, L, sigma2);
        else
            P = svd_precond(mvm, nd, sigma2, ceil(min(nd/50, 100)));
        end
        mvms = @(x) mvm(x) + sigma2 .* x; % Add lambda to sigma^2
        alpha = pcg(mvms, y(:), 1e-3, 1000, P);
    else % Scaling
        if ~isempty(dd) && ~isempty(get_row)
            scaled_row_handle = @(k) (S(k) * vec(get_row(k)) .* S)';
            [L, pp] = pchol_handles(scaled_row_handle, S .* dd .* S, 1e-6, ceil(min(nd/10, 400)));
            P = pchol_solve(pp, L, sigma2);
        else
            P = svd_precond(S*mvm(S*x), nd, S .* sigma2 .* S, ceil(min(nd/50, 100)));
        end
        alpha = solve_scaled_system(mvm, S, sigma2, y(:), P);
    end
else
    if isempty(S) % No scaling
        mvms = @(x) mvm(x) + sigma2 .* x; % Add lambda to sigma^2
        [alpha, flag, res] = pcg(mvms, y(:), 1e-3, 1000);
        if flag ~= 0, fprintf('Warning: pcg did not converge, residual = %.3e\n', res); end
    else % Scaling
        alpha = solve_scaled_system(mvm, S, sigma2, y(:), precond, 1e-3, 1000);
    end
end
...
term1 = y(:)'*alpha;
if nargout == 2
    [ld, dld] = logdet_lanczos(mvms, nd, Z, dmvm, min(nd/2, 50), 1);
else
